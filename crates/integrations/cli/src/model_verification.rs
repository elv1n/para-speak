...
use thiserror::Error;
use ml_utils::{get_model_cache_path, get_model_info, verify_model_files};
...
impl ModelVerification {
    pub fn new(model_name: String) -> Result<Self, ModelDownloadError> {
    ...
    pub fn verify_or_download(&self) -> Result<(), ModelDownloadError> {
        match verify_model_files(&self.model_name) {
            Ok(()) => {
                Ok(())
            }
            Err(err) => {
                eprintln!("\n🔍 Model verification failed, starting download...");
                eprintln!();

                if err.contains("File size mismatch") {
                    eprintln!("⚠️  Incomplete model files detected");
                    eprintln!("   {}", err);
                    eprintln!();
                ...
                eprintln!();
                eprintln!("✅ Model ready: {}", self.model_name);
                eprintln!();
    ...
    fn remove_incomplete_files(&self) -> Result<(), ModelDownloadError> {
        ...
        if snapshot_path.exists() {
            eprintln!("🗑️  Removing incomplete files...");
    ...
    fn download_model(&self) -> Result<(), ModelDownloadError> {
        eprintln!("📦 Downloading model: {}", self.model_name);
        eprintln!("   Calculating total download size...");
        eprintln!();
        ...
        if total_size > 0 {
            ...
            eprintln!("📦 Total download size: {:.2} GB", total_size_gb);
        }
        eprintln!();
        ...
        for (file_name, url, expected_size) in file_info {
            ...
            if dest_path.exists() {
                eprintln!("⏭️  {} already exists, skipping", file_name);
            ...
            let file_pb = if expected_size > 0 {
                ...
                eprintln!("  Downloading {} (size unknown)...", file_name);
                None
            };
        ...
        eprintln!();
        eprintln!("✅ Successfully downloaded model {}", self.model_name);
        eprintln!("   Model location: {:?}", snapshot_path);
        eprintln!();
